...
export default function Analytics() {
    ...
    // Normalizes both { name, count } and MongoDB { _id, count } shapes, plus null guards
    const normalizeAnalytics = (raw) => {
        if (!raw) return null;
        const summary = raw.summary || raw.kpis || raw;
        return {
            summary: {
                total: summary.total ?? 0,
                resolvedToday: summary.resolvedToday ?? summary.resolved ?? 0,
                criticalPending: summary.criticalPending ?? summary.critical ?? 0,
                resolutionRate: summary.resolutionRate ?? 0,
            },
            byCategory: (raw.byCategory || []).map(d => ({ name: d.name || d._id || 'Unknown', count: d.count ?? 0 })),
            byStatus: (raw.byStatus || []).map(d => ({ name: d.name || d._id || 'Unknown', count: d.count ?? 0 })),
            byDepartment: (raw.byDepartment || []).map(d => ({ name: d.name || d._id || 'Unknown', avgHours: d.avgHours ?? d.count ?? 0 })),
            overTime: (raw.overTime || []).map(d => ({ date: d.date || d._id || '', count: d.count ?? 0 })),
        };
    };
